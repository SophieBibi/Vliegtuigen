...
APP_NAME = "vliegtuigen_v6.py"
LUC_DATA_FILE = "luchthaven_data_6.xlsx"

st.set_page_config(
    page_title="Eindpresentatie Data science",
    page_icon="✈️",
    layout="wide"
)
...
def load_luchthaven_data(path: str = LUC_DATA_FILE) -> pd.DataFrame:
    ...
    # Extra vertaling van luchtvaartmaatschappijcodes.
    airline_map = {
        "2L": "Helvetic Airways",
        "A3": "Aegean Airlines",
        "AF": "Air France",
        ...
        "XX": "Onbekende maatschappij",
        "YW": "Air Nostrum",
    }
    df["maatschappij"] = df["maatschappij"].astype(str).str.strip()
    df["maatschappij_naam"] = df["maatschappij"].map(airline_map).fillna(df["maatschappij_naam"])

    # Labels met code + volledige naam maatschappijen en luchthavens
    df["luchthaven_label"] = (
        ...
        if option_menu:
            return option_menu(
                ...
                styles={
                    ...
                    "nav-link-selected": {
                        "background-color": KLEUREN["primary"],
                        "color": "#FFFFFF",
                        "font-weight": "700",
                    },
                },
...
def page_intro():
    st.title("Presentatie Vliegtuigdata")

    st.image("Vliegtuig stock photo.jpg")

    st.subheader("In deze presentatie: ")

    st.markdown("""
    - Deel 1: Vluchtdata vergelijken
    - Deel 2: Vertraging van vluchten analyseren
    """)

    st.markdown(
    """
    <p style='font-size:13px; color:gray;'>
    Gebruikte libraries:<br>
    • Streamlit<br>
    • Pandas<br>
    • Plotly Express<br>
...
def draw_flight_map(df: pd.DataFrame, show_speed: bool):
    ...
    legenda = "Snelheid in knopen" if show_speed else "Hoogte in feet"
...
def page_vluchten():
    st.title("7 vluchten van Amsterdam naar Barcelona")
    st.write("Op deze pagina vergelijken we een zevental vluchten op dezelfde route. Door de hoogte en snelheid gedurende de vluchten te analyseren worden de verschillen in het vluchtverloop inzichtelijk gemaakt.")
    ...
    st.subheader("Hoogte of snelheid gedurende de vlucht")
    ...
    y_label = "Snelheid (knopen)" if show_speed_line else "Hoogte (feet)"
    ...
    fig = px.line(
        ...
        y=y_col,
        color="vlucht",
        title=f"{y_label} gedurende de vlucht",
    ...
    st.write("Hoewel duidelijke verschillen tussen de vluchten zichtbaar zijn, ontbreekt aanvullende data om deze volledig te verklaren. Denk hierbij aan factoren zoals weersomstandigheden, windrichting of het totale gewicht van het vliegtuig.")
...
def page_vertragingen():
    ...
    # Top 20
    st.subheader("Is er een relatie tussen de vertraging van vluchten en de luchthaven waarop zij vertrekken of aankomen?")
    top_airports = (
    ...
    fig = px.bar(
        ...
        y="aantal_vluchten",
        hover_data=["Country"],
        title="Top 20 meest voorkomende luchthavens in de dataset",
    ...
    # Percentages per luchthaven
    st.subheader("Percentage vluchten te laat / op tijd / te vroeg per luchthaven")
    ...
    # Trend gemiddelde vertraging per maand
    st.subheader("Zichtbare trend in vertragingen per maand")
    maand_data = (
    ...
    st.subheader("Conclusie")
    st.write("Uit de analyse blijkt dat vertragingen grotendeels samenhangen met de gebruikte luchthaven. Sommige luchthavens hebben een duidelijk hoger percentage vertraagde vluchten dan andere. De gemiddelde vertraging per luchtvaartmaatschappij blijft relatief constant, met enkele uitschieters. Daarnaast is een opvallende trend zichtbaar tijdens de pandemie: zowel het aantal vluchten als de gemiddelde vertraging neemt af.")
