...
def page_vertragingen():
    ...
    if map_data.empty:
        ...
        # Terug naar de eenvoudige, stabiele kaart. Geen Mapbox en geen KLEUREN-dictionary.
        # Alleen de schaal is aangepast voor beter contrast.
        fig = px.scatter_geo(
            map_data,
            lat="Latitude",
            lon="Longitude",
        ...
        fig.update_traces(
            marker=dict(
                sizemode="area",
                sizeref=max(map_data["aantal_vluchten"].max() / 80, 1),
                sizemin=6,
        ...
        fig.update_layout(
            ...
            coloraxis_colorbar=dict(
                ...
                bordercolor="#1F4E79",
                borderwidth=1,
                tickfont=dict(color="#1F4E79", size=12),
                titlefont=dict(color="#1F4E79", size=13),
            ),
        )
