...
def page_vertragingen():
    ...
    map_data = (
        df.groupby(["luchthaven_label", "luchthaven_naam", "ICAO", "Country", "Latitude", "Longitude"])
        .agg(aantal_vluchten=("FLT", "count"), gemiddelde_vertraging=("verschil_minuten", "mean"))
        .reset_index()
    )

    # Geen Mapbox gebruiken: dit voorkomt fouten met nieuwere Plotly-versies op Streamlit Cloud.
    # De kaart gebruikt scatter_geo, met duidelijk contrast in de kaart en legenda.
    fig = px.scatter_geo(
        map_data,
        lat="Latitude",
        lon="Longitude",
        ...
        color_continuous_scale=[[0, KLEUREN["secondary"]], [0.5, KLEUREN["accent"]], [1, KLEUREN["danger"]]],
        labels={"gemiddelde_vertraging": "Gem. vertraging (min)"},
    )
    fig.update_geos(
        visible=True,
        showcountries=True,
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(
            ...
            borderwidth=1,
            tickfont=dict(color=KLEUREN["primary"], size=12),
            titlefont=dict(color=KLEUREN["primary"], size=13),
        ),
    )
